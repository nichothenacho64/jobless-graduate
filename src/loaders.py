...
def resolve_spreadsheet_path(folder: str | Path, file_name: str) -> Path:
...
def open_excel_file(folder: str | Path, file_name: str) -> tuple[Path, pd.ExcelFile]:
    file_path = resolve_spreadsheet_path(folder, file_name)
    excel_file = pd.ExcelFile(file_path)
    return file_path, excel_file

def list_excel_sheets(folder: str | Path, file_name: str) -> list[str]:
    _, excel_file = open_excel_file(folder, file_name)
    return cast(list[str], excel_file.sheet_names)

def load_excel_sheet(folder: str | Path, file_name: str, sheet_name: str) -> pd.DataFrame:
